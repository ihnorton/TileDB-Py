...
public:
  ...
  void update_read_elem_num() {
    for (const auto &read_info : query_->result_buffer_elements()) {
      ...
      // TODO need to support mixed offset mode here - do for 32-bit
      // update offsets due to incomplete resets
      auto offset_ptr = buf.offsets.mutable_data();
      //auto offset_ptr = (uint32_t*)buf.offsets.mutable_data();
        ...
        if (buf.offsets_read > 0) {
          if (offset_ptr[buf.offsets_read] == 0) {
            ...
            for (uint64_t i = 0; i < offset_elem_num; i++) {
              offset_ptr[buf.offsets_read + i] += last_size; // + last_offset;
            }
          }
        }
      }
      ...
      //result_buffer_elements is wrong for 32-bit mode
      //buf.offsets_read += offset_elem_num; // use_arrow_ ? (offset_elem_num - 1) : offset_elem_num;
      //buf.offsets_read += (offset_elem_num * 2);
    }
  }
